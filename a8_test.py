...
class TestGrid:
    ...
    def test_print_grid(self):
        ...
        cell2 = grid.get_cell(1,0)
        cell3 = grid.get_cell(0,1)
        cell1.link(cell2)
        cell1.link(cell3)
        grid.print()
...
class TestSidewinder:
    ...
    def test_SidewinderMazeMaker_basic(self):
        grid = Grid(25, 25)
        grid.print()
        maze = SidewinderMazeMaker(grid)
        grid.print()
        grid.export_image("s_maze_basic.png")
        solver = DjikstraSolver(grid)
        path = solver.solve()
        for ind, cell in enumerate(grid):
            cell.content = cell.distance
        print(path)
        grid.export_image("s_maze_basic_solved.png", path)

    def test_SidewinderMazeMaker_basic_2(self):
        for i in range(100):
            grid = Grid(25, 25)
            #grid.print()
            maze = SidewinderMazeMaker(grid)
            ...
            print(i)
            filename = str(i)+".png"
            grid.export_image(filename, path)
